...
{
	int start_vertex = 0;
	visited = vector<char>(number_of_vertices, false);
	dfs(start_vertex);
...
void dijkstra_via_priority_queue()
{
	...
	std::priority_queue< pair<int, int>, std::vector< pair<int, int> >, std::greater<pair<int, int> > > discovered;
	int start_vertex = 0;
	dist[start_vertex] = 0;

	while(!discovered.empty())
	{
		pair<int, int> nearest = discovered.top();
		discovered.pop();

		int vertex = top.second, nearest_dist = top.first;
		...
		if(nearest_dist <= dist[vertex])
		{
			for(const auto& el : graph[vertex])
			{
				...
				if(dist[vertex2] > dist[vertex] + cost)
				{
					...
					discovered.push(make_pair(dist[vertex2], v2));
				}
			}
		}
